...
export const Stage3D: React.FC = () => {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if (dragMode === 'rotate') {
      const nextX = Math.max(-85, Math.min(85, threeDConfig.rotateX - dy * 0.4));
      const nextY = (threeDConfig.rotateY + dx * 0.4) % 360;
  ...
  const handleWheel = (e: React.WheelEvent) => {
    ...
    const nextZoom = Math.max(0.15, Math.min(3.0, threeDConfig.zoom * zoomDelta));
  ...
  const setOrientationPreset = (preset: 'isometric' | 'front' | 'top' | 'side' | 'perspective') => {
    switch (preset) {
      case 'isometric':
        setThreeDConfig({ rotateX: 45, rotateY: -45, panX: 0, panY: 0, autoRotate: false });
        break;
      case 'perspective':
        setThreeDConfig({ rotateX: 30, rotateY: -35, panX: 0, panY: 0, autoRotate: false });
        break;
      case 'front':
        setThreeDConfig({ rotateX: 0, rotateY: 0, panX: 0, panY: 0, autoRotate: false });
        break;
      case 'top':
        setThreeDConfig({ rotateX: 85, rotateY: 0, panX: 0, panY: 0, autoRotate: false });
        break;
      case 'side':
        setThreeDConfig({ rotateX: 0, rotateY: -90, panX: 0, panY: 0, autoRotate: false });
        break;
    }
  };
  ...
  return (
    <div
      ...
      className="w-full h-full relative overflow-hidden flex items-center justify-center select-none bg-[#07080c] cursor-grab active:cursor-grabbing"
      style={{
        perspective: `${threeDConfig.perspective}px`,
    ...
    >
      ...
      >
        ...
        <div
          className="absolute inset-0 rounded-lg border-2 border-indigo-500/30 transition-all"
          style={{
            transformStyle: 'preserve-3d',
            transform: 'translateZ(0px)',
            backgroundColor: canvas.backgroundColor || '#090a0f',
            boxShadow:
              '0 30px 80px rgba(0, 0, 0, 0.9), 0 0 40px rgba(99, 102, 241, 0.15)',
          }}
        >
          {/* Subtle Grid on Floor */}
          <div
            className="w-full h-full absolute inset-0 opacity-25 pointer-events-none rounded-lg"
            style={{
              backgroundImage:
          ...
          <div className="absolute top-1/2 left-0 right-0 h-[1px] bg-indigo-500/20 pointer-events-none" />
          <div className="absolute left-1/2 top-0 bottom-0 w-[1px] bg-indigo-500/20 pointer-events-none" />

          {/* Base Plane Label */}
          <div className="absolute bottom-4 left-4 font-mono text-[11px] text-slate-500 bg-[#11131a]/80 px-2 py-1 rounded border border-white/5 pointer-events-none">
            Canvas Base Floor (Z: 0px) • {canvas.width}×{canvas.height}
          </div>
        </div>
        ...
        {sortedLayers.map((layer) => {
          ...
          const zElevation = (layer.trackIndex + 1) * threeDConfig.layerSpacing;

          const layerW = Math.max(10, transform.width);
          const layerH = Math.max(10, transform.height);
          ...
          return (
            ...
            >
              ...
              {threeDConfig.showLayerBadges && (
                <div
                  className="absolute -top-7 left-0 flex items-center gap-1.5 px-2 py-0.5 rounded bg-[#11131a]/90 backdrop-blur-md border border-[#222734] shadow-lg pointer-events-none text-[10px] font-mono whitespace-nowrap"
                  style={{
                    transformStyle: 'preserve-3d',
                ...
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${
                      isSelected ? 'bg-indigo-400' : 'bg-cyan-400'
                    }`}
                  />
                  <span className="font-bold text-slate-200">
                    T{layer.trackIndex + 1}: {layer.name}
                  </span>
                  <span className="text-slate-500">+{zElevation}px</span>
                </div>
              )}
        ...
        >
          ...
          >
            ...
            <div
              ...
              className="absolute inset-0 bg-indigo-900/80 border border-indigo-400/60 text-[9px] font-mono font-bold flex items-center justify-center text-white cursor-pointer hover:bg-indigo-600 transition-colors shadow-lg"
            ...
            {/* Back */}
            <div
              className="absolute inset-0 bg-slate-900/80 border border-slate-700 text-[9px] font-mono flex items-center justify-center text-slate-400 shadow-lg"
            ...
            <div
              ...
              className="absolute inset-0 bg-cyan-900/80 border border-cyan-400/60 text-[9px] font-mono font-bold flex items-center justify-center text-cyan-200 cursor-pointer hover:bg-cyan-600 transition-colors shadow-lg"
            ...
            <div
              ...
              className="absolute inset-0 bg-purple-900/80 border border-purple-400/60 text-[9px] font-mono font-bold flex items-center justify-center text-purple-200 cursor-pointer hover:bg-purple-600 transition-colors shadow-lg"
      ...
      <div className="absolute bottom-5 left-1/2 -translate-x-1/2 z-20 bg-[#11131a]/90 backdrop-blur-md px-4 py-2.5 rounded-2xl border border-[#222734] shadow-2xl flex items-center gap-4 text-xs">
        {/* Camera Presets */}
        <div className="flex items-center gap-1 bg-[#181b24] p-1 rounded-xl border border-[#222734]">
          <button
            ...
            className="px-2.5 py-1 rounded-lg text-[11px] font-semibold text-slate-300 hover:text-white hover:bg-slate-800 transition-colors"
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <span className="text-[11px] font-mono text-indigo-300 w-8">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className={`p-1.5 rounded-lg text-[11px] font-semibold transition-colors flex items-center gap-1 ${
              threeDConfig.autoRotate
                ? 'bg-amber-950/60 border border-amber-500/50 text-amber-300 shadow-glow-accent'
          ...
          <button
            ...
            title="Toggle Depth Projection Lines"
          >
      ...
      <div className="absolute top-4 left-4 z-10 bg-[#11131a]/85 backdrop-blur-md px-3 py-1.5 rounded-xl border border-[#222734] shadow-lg text-[11px] font-mono text-slate-400 flex items-center gap-2">
        <Rotate3d className="w-3.5 h-3.5 text-indigo-400 animate-spin-slow" />
        <span>3D Exploded Layer View</span>
        <span className="text-slate-600">•</span>
        <span className="text-slate-500">Drag to Orbit • Shift+Drag to Pan • Scroll to Zoom</span>
      </div>
    </div>
  );
};
