...
export const Viewport: React.FC = () => {
  ...
  }, [canvas.width, canvas.height, stageMode, setViewportZoom, setViewportPan]);
  ...
  return (
    ...
    >
      {/* Top Center: 2D Stage vs 3D Layer Stack Viewport Mode Tabs */}
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 flex items-center bg-[#11131a]/90 backdrop-blur-md p-1 rounded-2xl border border-[#222734] shadow-2xl">
        <button
          onClick={() => setStageMode('2d')}
          className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all ${
            stageMode === '2d'
              ? 'bg-gradient-to-r from-indigo-600 to-indigo-500 text-white shadow-glow-accent'
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all ${
            stageMode === '3d'
              ? 'bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white shadow-glow-accent animate-pulse-subtle'
              : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/60'
          }`}
        >
          <Rotate3d className="w-3.5 h-3.5" />
          <span>3D Layer Space</span>
          <span className="text-[9px] font-extrabold bg-indigo-400/20 text-indigo-300 px-1 py-0.2 rounded border border-indigo-400/30">
            PRO
          </span>
        </button>
      </div>

      {/* Top Floating Viewport Toolbar (2D mode controls) */}
      {stageMode === '2d' && (
      ...
      {isRecordingMotionPath && (
        <div className="absolute top-16 left-1/2 -translate-x-1/2 z-30 bg-rose-950/90 border border-rose-500/80 backdrop-blur-md px-4 py-2 rounded-2xl shadow-glow-accent flex items-center gap-3">
      ...
      {/* Interactive Center Stage: 2D or 3D */}
      {stageMode === '2d' ? <CanvasStage /> : <Stage3D />}

      {/* Interactive Minimap (2D mode) */}
      {stageMode === '2d' && showMinimap && (
        <Minimap
